...
function createCookie(key, value, expDays) {
    let date = new Date();
...
function setCookie() {
    let cookieButton = $("#cookie-accept-button");
    let cookieBanner= $("#cookie-banner");
    const accept = "accepted";

    $(cookieButton).click(function () {
        createCookie('PaS-cookie-consent', accept, 180);
        cookieBanner.removeClass("slideInBanner");
        setTimeout(function () {
            cookieBanner.remove();
        },1000)
    })

    if(getCookie("PaS-cookie-consent") === "") {
        cookieBanner.addClass("slideInBanner");
    }
}
